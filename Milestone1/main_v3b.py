...
class Leaf(Node):
    ...
    def predict(self, X):
        return np.full(X.shape[0], self.label)  # ✅ Array de labels
...
class Parent(Node):
    ...
    def predict(self, X):
        ...
        y[left_mask] = self.left_child.predict(X[left_mask])
        y[~left_mask] = self.right_child.predict(X[~left_mask])
        return y
...
rf = RandomForestClassifier(num_trees, min_size_split, max_depth, ratio_samples, num_random_features, criterion)#Train the model
# train = make the decision trees
rf.fit(X,y) 
# classification           
ypred = np.array([tree.predict(X) for tree in rf.trees])
ypred = np.mean(ypred, axis=0) > 0.5 # majority voting
# compute accuracy
