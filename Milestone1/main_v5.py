...
class RandomForestClassifier:
    ...
    def _CART_cost(self, left_dataset, right_dataset): 
        ...
        total = left_dataset.get_num_samples + right_dataset.get_num_samples #total number of samples
        if total == 0:
            logging.warning('Total number of samples is zero') 

        cost = abs(left_dataset.get_num_samples/total)*self.criterion.method(left_dataset) + \
                abs(right_dataset.get_num_samples/total)*self.criterion.method(right_dataset)  
        return cost
